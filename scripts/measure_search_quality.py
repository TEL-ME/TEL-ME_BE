...
def self_test() -> int:
    ...
    total = len(SELF_TEST_CASES) + len(CATEGORY_SELF_TEST_CASES)
    print(f"\n자기 검증 {total}건 중 {failures}건 실패")
...
def main() -> int:
    ...
    outcomes = evaluate(eval_items, args.top_k, args.api_url, args.timeout, raw_sink)
    if raw_sink is not None:
