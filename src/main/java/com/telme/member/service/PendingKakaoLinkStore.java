...
public class PendingKakaoLinkStore {

    public static final String SESSION_ATTRIBUTE = "pendingKakaoLink";
    ...
    public void issue(HttpServletRequest request, String providerUserId, Long matchedUserId, String matchedEmail) {
        request.getSession().setAttribute(
                SESSION_ATTRIBUTE, PendingKakaoLink.issue(providerUserId, matchedUserId, matchedEmail, clock.instant()));
    }

    // 만료·시도횟수 초과면 바로 제거하고 예외 — 오래전에 인증한 카카오 계정이 엉뚱한 시점에 연결되는 것을 막는다
    public PendingKakaoLink require(HttpServletRequest request) {
        HttpSession session = request.getSession(false);
        PendingKakaoLink pending = session == null ? null : (PendingKakaoLink) session.getAttribute(SESSION_ATTRIBUTE);
    ...
    public void registerFailedAttempt(HttpServletRequest request, PendingKakaoLink pending) {
        HttpSession session = request.getSession(false);
        if (session == null) {
            return;
        }
        PendingKakaoLink updated = pending.withFailedAttempt();
