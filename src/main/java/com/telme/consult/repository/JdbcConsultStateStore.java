    ...
    private Snapshot read(long sessionId, long requestId) {
        var requests =
                jdbc.query(
                        ...
                        requestId,
                        sessionId);
        if (requests.isEmpty()) throw new IllegalArgumentException("Consult request unavailable");
        ...
        return tx.execute(
                txStatus -> {
                    ...
                    if (asks) {
                        ...
                        if (later != 1)
                            throw new IllegalArgumentException(
                                    "Clarification must follow its origin question");
                    }
                    if (links.answerMessageId() != null) {
                        ...
                        if (value == null || value.status() == ConditionStatus.PENDING)
                            throw new IllegalArgumentException(
                                    "Answer must fill or decline its condition");
                        var asked =
                                jdbc.queryForList(
                                        "SELECT asked_message_id FROM consult_conditions WHERE"
                                            + " consult_request_id=? AND condition_key=? AND"
                        ...
                        if (asked.isEmpty())
                            throw new IllegalArgumentException(
                                    "No pending clarification for field");
                        int later =
                                jdbc.queryForObject(
                        ...
                        if (later != 1)
                            throw new IllegalArgumentException(
                                    "Followup must be later than its clarification");
                    }
                    for (var entry : conditions.entrySet()) {
                        ...
                        jdbc.update(
                                "INSERT INTO consult_conditions"
                                    + " (consult_request_id,condition_key,condition_value,status,source)"
                                    + " VALUES (?,?,?,?,?) ON CONFLICT"
                                    + " (consult_request_id,condition_key) DO UPDATE SET"
                                    + " condition_value=EXCLUDED.condition_value,status=EXCLUDED.status,updated_at=now()",
                                old.requestId(),
                                key,
                                c.value(),
                                c.status().name(),
                        ...
                        if (!isAsked
                                && !isAnswered
                                && !Objects.equals(c, old.conditions().get(key))) {
