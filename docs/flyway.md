...
| `src/main/resources/db/dev-migration` | 로컬 개발용 샘플 데이터 (`V2__seed_sample_data.sql`) | **적용 안 됨** |
...
파일명 규칙은 `V{버전}__{설명}.sql`이고, 버전 번호는 두 폴더를 합쳐서 유일해야 합니다(현재 1~2 사용 중, 다음 스키마 변경은 `V3__...`).

> 초기 스키마는 원래 `V1`~`V8`로 나뉘어 있었지만, 머지 전까지 Flyway를 쓴 사람이 담당자뿐이고 배포된 환경도 없어서 `V1__init_schema.sql` 하나로 합쳤습니다. `V8`이 `V1`~`V6`이 만든 인덱스·CHECK를 되돌리는 구조라 이력을 남길 실익이 없었습니다. **develop에 머지된 뒤로는 이런 통합이 불가능하므로**, 앞으로 스키마를 바꿀 때는 반드시 새 버전 파일을 추가하세요.
...
- **이미 적용된 버전 파일(V1~V2)은 절대 내용을 고치지 마세요.** `flyway validate`가 체크섬 불일치로 바로 실패합니다. 고칠 게 있으면 새 버전 파일(`V3__...sql`)을 추가하세요. Flyway Community Edition은 자동 롤백(undo)을 지원하지 않아서, 되돌리기도 "새 마이그레이션으로 되돌리는 SQL을 추가"하는 방식입니다.
